...
def do_install(
    ...
    if version:
        ...
        available = [v for v in all_available if (allow_snapshots or not v.is_snapshot())]
    ...
    if not installed:
        print(f"Installed {dest}. No prior version existed, so configuration must be completed manually")
        return
